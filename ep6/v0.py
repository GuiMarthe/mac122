...
def distancia(p0, p1):
    ...
    R = math.sqrt((p0[0] - p1[0])**2 + (p0[1] - p1[1])**2)
    return R
    ...
    if len(lista_pontos) == 0:
        ...
        for i in range(p, r):
            for j in range(p, r):
                ...
                if i != j and dist < dist_min:
                    closest_points[0], closest_points[1] = lista_pontos[i], lista_pontos[j] 
                    dist_min = dist
    return dist_min, closest_points
...
def par_mais_proximo_faixa(d, p, q, r, lista_pontos):
    ...
    d_min = None
    R = []
    centro_x = lista_pontos[q][0]
    for i in range(p,q):
        if lista_pontos[i][0] > centro_x - d:
            for j in range(r-1,q-1,-1):
                if lista_pontos[j][0] < centro_x + d:
                    dist = distancia(lista_pontos[i], lista_pontos[j])
                    if d_min is None:
                        d_min = dist
                        R.append(lista_pontos[i])
                        R.append(lista_pontos[j])
                    elif d_min > dist:
                        d_min = dist
                        R[0] = lista_pontos[i]
                        R[1] = lista_pontos[j]
...
def mac0122(p, r, lista_pontos):
    ...
    if len(lista_pontos[p:r]) == 3:
        ...
        if dist1 > dist2:
            return dist2, [lista_pontos[q], lista_pontos[r-1]]
        else:
